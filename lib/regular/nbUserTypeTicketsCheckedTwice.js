...
module.exports = function (startDate, userType) {
  return function () {
    ...
    var ctx = this;
    var db = ctx.db;
    var nbUserTypeTicketsChecked = require('./nbUserTypeTicketsChecked')(startDate, userType, 2);
    ...
    .and.whereNotNull('cd_applications.attendance')
    .and.whereRaw('cd_applications::text != \'{}\'')
    .groupBy('cd_applications.user_id')
    .havingRaw('count(*) > 1')
    .then( function (rows) {
      if (ctx.output) {
        fs.appendFileSync(ctx.filename, '\nCount of ticket checkedIn twice for ' + userType + ' since ' + startDate + ': ' + rows.length + '\n');
      }
