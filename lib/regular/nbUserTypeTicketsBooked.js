 ...
 */
module.exports = (startDate, userType, minAttendances) => function () {
  ...
  return req.then(rows => {
    if (ctx.output) {
      fs.appendFileSync(ctx.filename, `\nCount of ticket booked at least ${(minAttendances || 0) + 1} times for ${userType} since ${startDate}: ${rows.length}\n`);
    }
