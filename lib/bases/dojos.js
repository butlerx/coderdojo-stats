...
module.exports = dojosDB => ({
    ...
    return dojosDB('cd_usersdojos')
      ...
      .whereIn(
        ...
        dojosDB
          ...
          .from(dojosDB('cd_usersdojos').select(dojosDB.raw('unnest(user_types) as user_type, user_id')).as('associations'))
  ...
  byRawRoleType (fields, role) {
    return dojosDB
      .select(dojosDB.raw(fields))
      .from(dojosDB('cd_usersdojos').select(dojosDB.raw(`unnest(user_types) as user_type, ${fields}`)).as('associations'))
      .where('associations.user_type', '=', role);
  },
  ...
  activeDojos (since, verifiedAt) {
    const q = dojosDB('cd_dojos').select('id', 'name').where('verified', '=', 1).andWhere('deleted', '=', 0).andWhere('stage', '!=', 4);
