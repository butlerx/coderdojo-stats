        ...
        dojosClient.query('SELECT user_id FROM cd_usersdojos ud INNER JOIN cd_dojos d ON ud.dojo_id = d.id ' +
          ...
          function (err, users) {
            ...
            if (users && users.rows.length > 0) {
              ...
              const filteredUsers = _.filter(legacyUsers, function (e) {
            ...
            return usersDB.select('sys_user.email', 'sys_user.name', 'sys_user.mailing_list', 'cd_profiles.alpha2').from('sys_user').join('cd_profiles', 'sys_user.id', '=', 'cd_profiles.user_id').whereIn('sys_user.id', champions).andWhere('sys_user.mailing_list', 1)
              ...
              .then(function (championsProfiles) {
                ...
                if (championsProfiles.length > 0) {
                  const csv = json2csv({ data: championsProfiles });
                  const fileName = userType + (countryCode || '') + 'Newsletter' + (excludedCountries && excludedCountries.length > 0 ? '_excluding_' + excludedCountries.join('_') : '') + '.csv';
...
function getDojoAndChampionEmails (countryCode) {
  return function () {
    return new Promise(function (resolve1, reject1) {
      const csvRows = [];
      const promiseChain = dojosDB.select('id', 'email', 'name', 'alpha2').from('cd_dojos').where('verified', '=', 1).andWhere('deleted', '=', 0).andWhere('stage', '!=', 4);
      if (countryCode) {
